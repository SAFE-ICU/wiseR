...
Network analysis is a popular method which can help in visualization, representation and gaining useful insights from complex data which enables quantitative decision making from the same. However the most basic form of network analysis i.e association network are useful only to the extent of basic insights by forming correlation structures between different variables, but not good enough to produce quantitative insights as it is not a learning model. For this purpose we have bayesian networks.

Bayesian Networks (BNs) are a probabilistic graphical models that can provide a single joint-multivariate fit on the data. The model structure is a list of conditional independencies represented as a directed acyclic graph. Fitting a joint multivariate graphical model accounts for conditional independencies, thus minimizing confounding effects. It also minimizes false discoveries by eliminating the need to conduct pairwise measures for constructing association networks. Finally, unlike black-box models, BN artificial intelligence models are interpretable, can be learnt directly from data, allow expert knowledge to be injected at all stages of model building and can reveal actionable measures through inference algorithms. Bayesian Network Inference can be effected on observing the state of a variable.Due to these properties, BNs have been extensively used as artificial intelligence expert systems for guiding decisions.

## Actionable points in bayesian structure
People sometimes find it difficult to understand bayesian structures and often misinterpret it to function like association networks. That is quite not the case. While association networks represents basic correlations between variables it is not the same with bayesian networks. Bayesian networks are directed graph and hence represent parent child relationships between variables based on the direction of the arc. The graph is also used to understand conditional independence between variables. These properties along with inference learning from the final structure make them such a useful tool in decision making.

Some of the key structures to look for in bayesian network are:

### V-structure
The presence of a V-structure with two edges pointing towards a node represents competing hypotheses and observing one can explain away the other (inter-causal reasoning).

### Chains and forks
A chain represents sequential flow of belief and can be identified as a sequence of parent-child relationships from the start node to the end node.

A fork indicates independent variables with a common parent and can be identified as single parent node to 2 or more children not connected to each other, thus any 2 children forming a fork structure are independent of each other.
...
The wiseR engine is powered by the following packages
