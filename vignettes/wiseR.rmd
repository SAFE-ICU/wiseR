...
Calling the wiseR() function from the **_wiseR_** package launches the application in the default browser. The recommended browser for wiseR is Chrome. On launching the application for the first time, it conducts background checks, which takes 5-10 seconds depending upon the machine. This delay only happens on the first launch and all subsequent launches take less than a couple of seconds to initialize the user interface.  
...
#### Figure S6. The data pre-processing sub-section of Data tab allows imputation, discretization, type-conversion, variable deletion and transpose.
...
**Download graph**:User also has the option to download the network graph in its current state using the save network option present in the bottom right corner of the graph.
...
The data obtained from association networks analysis can be exported as an edgelist in tabular form and can also be downloaded it as a .CSV file.
...
The user may simply choose the "Direct Learning" tab for learning the structure without bootstraps, however this is conisdered lesss or do a bootstrapped one(in which case bootstrap parameters will be used) for more robust structure learning.
